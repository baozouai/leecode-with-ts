 ...
 * @LastEditTime: 2020-09-03 14:01:54
 */
...
function strStr(haystack: string, needle: string): number {
  // 获取主串与模式串的长度
  const originLength: number = haystack.length;
  const aimLength: number = needle.length;
  if (originLength < aimLength) {
    return -1;
  }
  // 声明主串与模式串的索引
  let originIndex: number = 0;
  let aimIndex: number = 0;
  // 当模式串未比较到最后一位
  while (aimIndex < aimLength) {
    ...
    if (haystack[originIndex] === needle[aimIndex]) {
       ...
       * */
      const nextCharIndex: number = originIndex - aimIndex + aimLength;
      // 判断该索引是否已超出主串
      if (nextCharIndex < originLength) {
        // 获取该索引对应元素在模式串最右的索引
        const rightestIndex: number = needle.lastIndexOf(haystack[nextCharIndex]);
        // 如果存在
        if (rightestIndex !== -1) {
          /**
