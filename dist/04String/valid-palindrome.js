...
function isPalindrome(s) {
     ...
     * 2.replace掉全部非数字|字母的
     */
    let tempStr = s.toLowerCase().replace(/[^0-9a-z]/g, '');
    // 判断正序与反序是否相同
...
function isPalindrome1(s) {
    ...
    while (i < j) {
        // 不是0-9，a-z则跳过
        while (!(reg.test(tempStr[i])))
            ++i;
        while (!(reg.test(tempStr[j])))
            --j;
        // 有一个不相等则false
...
function isPalindrome2(s) {
    ...
    while (i < j) {
        // 不是0-9，a-z则跳过
        while (!((tempStr[i] >= '0' && tempStr[i] <= '9') || (tempStr[i] >= 'a' && tempStr[i] <= 'z')))
            ++i;
        while (!((tempStr[j] >= '0' && tempStr[j] <= '9') || (tempStr[j] >= 'a' && tempStr[j] <= 'z')))
            --j;
        // 有一个不相等则false
        if (tempStr[i++] !== tempStr[j--]) {
            return false;
